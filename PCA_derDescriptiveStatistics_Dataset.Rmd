...
```{r}
RNA_pca_var <- RNA_pca$sdev^2
RNA_pca_var
```

```{r}
RNA_pca_ve <- RNA_pca_var / sum(RNA_pca_var)
RNA_pca_ve
```
```{r}
...
wss <- sapply(1:10, function(k){
  kmeans(RNA_pca$x, centers = k, nstart = 10)$tot.withinss
})
...
```{r}
kmeans_result <- kmeans(RNA_pca$x, centers = 6, nstart = 25)
```
