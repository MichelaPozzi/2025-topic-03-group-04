...
```{r}
# look up, if Protein is in data
ergebnis <- normalized_ctrl_df[grep("CTCF", normalized_ctrl_df[[1]]), ]
...
df_joined <- df_joined %>%
  filter(is.finite(Global_Maximum_RNase) & is.finite(Global_Maximum_Controll))


# Differenz berechnen und im selben df speichern
df_joined <- df_joined %>%
  mutate(diff = Global_Maximum_RNase - Global_Maximum_Controll)

# Plot vorbereiten
max_val <- max(df_joined$Global_Maximum_RNase, df_joined$Global_Maximum_Controll) * 1.1
colors <- ifelse(df_joined$diff > 0, "pink", ifelse(df_joined$diff < 0, "lavender", "cyan"))

# Scatterplot
plot(
  df_joined$Global_Maximum_RNase, df_joined$Global_Maximum_Controll,
