...
```{r}
#compute averages per fraction (separatly for Ctrl and RNAse)
...
for (r in 1:25) {
  ...
  number_replicates <- 3
  #defines which columns are relevant (starting with the second column; column 1 consists of protein names -> irrelevant for average)
  replicates_indices <- seq(2, ncol(df), by = group_size) 
  
  average_values <- sapply(replicates_indices, function(j) {
    #mean of rows is computed starting in column j ending in column (j + number_replicates -1)
    #combined with replicates_indices the averages are computed for columns 2-4 and 5-7
    rowMeans(df[, j:(j + number_replicates - 1)])
  })
  
  #creates general makeup of dataframe containg averages
  averaged_df <- as.data.frame(average_values)
  colnames(averaged_df) <- c("Mean_CTRL", "Mean_RNASE")
  averaged_df$Protein_Name <- df$Protein_Name
  averaged_df$fraction <- paste0("fraction", r)
  
  #for each fraction the respective subdataframe is put into averages_list
  averages_list[[r]] <- averaged_df
}

# list with means for all proteins across all fractions (column 4 containes fractionnames)
final_df <- do.call(rbind, averages_list)
