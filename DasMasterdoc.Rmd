...
# 3D-Plot mit lila eingefärbten Datenpunkten nach RBP oder Non-RBP -> macht vielleicht nach einem anderen Parameter mehr Sinn
#Einteilung der Proteine anhand von threshold in RBPs und Non-RBPs
# Klassifikation basierend auf optimal_threshold
# 1. Klassifikation hinzufügen, falls noch nicht passiert
df_complete$predicted_RBP <- ifelse(df_complete$probability >= optimal_threshold,
                                    "RBP", "Non-RBP")

# 2. Plotly-Plot
plot_ly() %>%
  # Datenpunkte: Farbe nach RBP-Klassifikation (nicht Wert)
  add_markers(data = df_complete,
              x = ~pI,
              y = ~Mass_kDa,
              z = ~probability,
              color = ~predicted_RBP,             # ← Klassifikation statt Wert
              colors = c("RBP" = "purple", 
                         "Non-RBP" = "lightgray"), # ← 2 Farben für Klassen
              marker = list(size = 4),
              name = "Klassifikation",
              showlegend = TRUE) %>%
  # Regressionsfläche (weiterhin violett)
  add_surface(x = ~pI_seq,
              y = ~Mass_seq,
              z = matrix(grid$predicted, nrow = 30),
              opacity = 0.5,
              colorscale = "Purples",
              showscale = FALSE,  # Skala nur für Fläche
              name = "Regressionsfläche") %>%
  # Achsenbeschriftungen
  layout(scene = list(
    xaxis = list(title = "pI"),
    yaxis = list(title = "Masse (kDa)"),
    zaxis = list(title = "Probability")
  ),
  legend = list(title = list(text = "<b>RBP-Klassifikation</b>")))
