...
###pca   
```{r}
ctrl_sva_df <- corrected_expr_ctrl[,2:ncol(corrected_expr_ctrl)]
str(ctrl_sva_df)

apply(ctrl_sva_df, 2, mean)
...
Ctrl_pca_var <- RNA_pca$sdev^2 # eigenvalues 
Ctrl_pca_var

Ctrl_pca_ve <- round(Ctrl_pca_var / sum(Ctrl_pca_var)*100,2) # wert in prozent auf zwei dezimal 
Ctrl_pca_ve
...
###weirder plot 
```{r}
biplot(Ctrl_pca, scale = 0, cex = 0.4)
```
...
###pca selbst  
```{r}
rnase_sva_df <- corrected_expr_rnase[,2:ncol(corrected_expr_rnase)]
str(rnase_sva_df)

apply(rnase_sva_df, 2, mean)
...
rnase_pca_ve <- round(rnase_pca_var / sum(rnase_pca_var)*100,2) # wert in prozent auf zwei dezimal 
rnase_pca_ve
...
###weirder plot 
```{r}
biplot(rnase_pca, scale = 0, cex = 0.4)
```
...
##rnase 
```{r}
R_rsp_sva_df <- right_shift_rnase[,2:ncol(right_shift_rnase)]
str(R_rsp_sva_df)

apply(R_rsp_sva_df, 2, mean)

R_rsp_pca <- prcomp(R_rsp_sva_df, center = TRUE, scale = TRUE)
...
R_rsp_pca_var <- R_rsp_pca$sdev^2 # eigenvalues 
R_rsp_pca_var

R_rsp_pca_ve <- round(R_rsp_pca_var / sum(R_rsp_pca_var)*100,2) # wert in prozent auf zwei dezimal 
R_rsp_pca_ve

sort(R_rsp_pca_ve,decreasing=TRUE)
```

###prozentplot 
```{r}
barplot(R_rsp_pca_ve)
```

###weirder plot 
```{r}
biplot(R_rsp_pca, scale = 0, cex = 0.4)
```
...
# Nur die numerische Matrix aus dem PCA-Objekt extrahieren
R_rsp_pca_data <- R_rsp_pca$x  # oder ggf. pca_data <- na.omit(Ctrl_pca$x)

sil_width <- numeric(9)

for(k in 2:10) {
  km <- kmeans(R_rsp_pca_data, centers = k, nstart = 25)
  ss <- silhouette(km$cluster, dist(R_rsp_pca_data))  # dist() NUR auf pca_data
  sil_width[k - 1] <- mean(ss[, 3])
}
...
###kmeans 
```{r}
kmeans_result <- kmeans(R_rsp_pca$x, centers = 2, nstart = 25)
```
...
```{r}
R_rsp_pca_data <- as.data.frame(R_rsp_pca$x)
R_rsp_pca_data$cluster <- as.factor(kmeans_result$cluster)

library(ggplot2)

ggplot(R_rsp_pca_data, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(title = "K-Means Cluster im PCA-Raum",
       x = "Hauptkomponente 1 (PC1)",
       y = "Hauptkomponente 2 (PC2)") +
...
```{r}
first_two_components <- R_rsp_pca_data[,1:2]
plot(first_two_components)
```
...
```{r}
C_rsp_sva_df <- right_shift_ctrl[,2:ncol(right_shift_ctrl)]
str(C_rsp_sva_df)

apply(C_rsp_sva_df, 2, mean)

C_rsp_pca <- prcomp(C_rsp_sva_df, center = TRUE, scale = TRUE)
...
C_rsp_pca_var <- C_rsp_pca$sdev^2 # eigenvalues 
C_rsp_pca_var

C_rsp_pca_ve <- round(C_rsp_pca_var / sum(C_rsp_pca_var)*100,2) # wert in prozent auf zwei dezimal 
C_rsp_pca_ve

sort(C_rsp_pca_ve,decreasing=TRUE)
```

###prozentplot 
```{r}
barplot(C_rsp_pca_ve)
```

###weirder plot 
```{r}
biplot(C_rsp_pca, scale = 0, cex = 0.4)
```
...
#elbow 
wss <- sapply(1:25, function(k){
  kmeans(C_rsp_pca$x, centers = k, nstart = 25)$tot.withinss
})
...
# Nur die numerische Matrix aus dem PCA-Objekt extrahieren
C_rsp_pca_data <- C_rsp_pca$x  # oder ggf. pca_data <- na.omit(Ctrl_pca$x)

sil_width <- numeric(9)

for(k in 2:10) {
  km <- kmeans(C_rsp_pca_data, centers = k, nstart = 25)
  ss <- silhouette(km$cluster, dist(C_rsp_pca_data))  # dist() NUR auf pca_data
  sil_width[k - 1] <- mean(ss[, 3])
}
...
###kmeans 
```{r}
kmeans_result <- kmeans(C_rsp_pca$x, centers = 2, nstart = 25)
```
...
```{r}
C_rsp_pca_data <- as.data.frame(C_rsp_pca$x)
C_rsp_pca_data$cluster <- as.factor(kmeans_result$cluster)

library(ggplot2)

ggplot(C_rsp_pca_data, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(title = "K-Means Cluster im PCA-Raum",
       x = "Hauptkomponente 1 (PC1)",
       y = "Hauptkomponente 2 (PC2)") +
...
```{r}
first_two_components <- C_rsp_pca_data[,1:2]
plot(first_two_components)
```
...
loadings <- R_rsp_pca$rotation

# Standard-biplot zeichnen
biplot(R_rsp_pca, col = c("gray", "pink"))
...
loadings <- as.data.frame(R_rsp_pca$rotation[, 1:2])
