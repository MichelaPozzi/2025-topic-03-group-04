...
```{r}
pheatmap(expr_matrix[1:100, ], main = "Test Heatmap Original")
pheatmap(ctrl_expr_corrected[1:100, ], main = "Test Heatmap Korrigiert")
```
...
pca_df$Protein <- rownames(pca_df)  # Zeilennamen explizit in Spalte
rownames(pca_df) <- NULL            # Zeilennamen löschen, damit keine Verwirrung
...
```{r}
 ctrl_sva_df <- as.data.frame(corrected_expr_ctrl)
Bctrl_sva_df <- cbind(gene=rownames(ctrl_sva_df),ctrl_sva_df)

```

```{r}
T_ctrl_pca <- prcomp(ctrl_sva_df, center = TRUE, scale = TRUE)
T_ctrl_df<-cbind(gene=rownames(ctrl_sva_df),ctrl_sva_df)
```
...
# 2. Nur numerische Werte an PCA übergeben
C_pca <- prcomp(ctrl_sva_df, center = TRUE, scale = TRUE)

# 3. Ergebnis als DataFrame

C_pca_df <- as.data.frame(C_pca$x)
C_pca_df$Protein <- rownames(C_pca_df)  # Zeilennamen explizit in Spalte
rownames(C_pca_df) <- NULL            # Zeilennamen löschen, damit keine Verwirrung
...
# 6. Markierungsspalte
C_pca_df$Highlight <- ifelse(C_pca_df$Protein %in% highlight_proteins, "Ja", "Nein")
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Highlight)) +
  geom_point(size = 2, alpha = 0.7) +
  scale_color_manual(values = c("Ja" = "hot pink", "Nein" = "light gray")) +
  theme_minimal() +
  labs(title = "PCA Biplot mit hervorgehobenen Proteinen")
...
# Gitter für die Regressionsfläche
...
# Plotly Plot mit separaten Gruppen und eigener Legendenbeschriftung
fig <- plot_ly()
...
# RBP ohne Shift-Direction (optional)
fig <- fig %>%
