...
###weirder plot 
```{r}
biplot(rnase_pca, scale = 0, cex = 0.1)
```
...
```{r}
# PCA
T_rnase_pca <- prcomp(corrected_expr_rnase, center = TRUE, scale = TRUE)

# Liste der zu markierenden Proteine aus DataFrame
highlight_proteins <- right_shift_rnase$protein

# Loadings (z. B. für Proteinlabels)
loadings <- T_rnase_pca$rotation

# Standard-biplot zeichnen
biplot(T_rnase_pca, col = c("gray", "pink"))

# Labels einfügen – markierte Proteine hervorheben
text(loadings[,1]*150, loadings[,2]*150, labels = rownames(loadings),
     col = ifelse(rownames(loadings) %in% highlight_proteins, "blue", "black"),
     cex = ifelse(rownames(loadings) %in% highlight_proteins, 1.2, 0.6),
     font = ifelse(rownames(loadings) %in% highlight_proteins, 2, 1))
...
# PCA aus deinen Daten
T_rnase_pca <- prcomp(corrected_expr_rnase, center = TRUE, scale = TRUE)

# Ladeproteine extrahieren
loadings <- as.data.frame(T_rnase_pca$rotation[, 1:2])
loadings$protein <- rownames(loadings)

# Markierungen setzen
loadings$highlight <- loadings$protein %in% right_shift_rnase$protein

# Plot
ggplot(loadings, aes(x = PC1, y = PC2)) +
  geom_point(alpha = 0.2, color = "black") +  # alle Punkte
  geom_point(data = subset(loadings, highlight), color = "blue", size = 3) +  # markierte Punkte
  geom_text(data = subset(loadings, highlight), aes(label = protein), 
            color = "blue", size = 3, fontface = "bold", vjust = -1) +  # markierte Labels
  theme_minimal() +
  ggtitle("PCA: Nur markierte Proteine beschriftet") +
