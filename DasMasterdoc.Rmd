...
center_of_mass_df <- data.frame(
  ...
  COM_RNase   = com_rnase,
  Delta_COM   = com_rnase - com_control, # calculate delta
  Max_Delta_COM      = max_delta_com,
  Relative_Delta_COM = abs(com_rnase - com_control) / max_delta_com
)
...
#scale to 100% (max_delta = 10.9376)
center_of_mass_df$COM_scaled_100 <- (center_of_mass_df$COM_scaled/max_delta) * 100
head(center_of_mass_df$COM_scaled_100)
```
...
#elbow 
wss <- sapply(1:25, function(k){
  kmeans(R_rsp_pca$x, centers = k, nstart = 25)$tot.withinss
})
...
# Nur die numerische Matrix aus dem PCA-Objekt extrahieren
rnase_rsp_pca_data <- rnase_rsp_pca_pca$x  # oder ggf. pca_data <- na.omit(Ctrl_pca$x)
...
#try smth
# Nur Zeilen ohne fehlende Werte für pI, Mass_kDa und testscores
df_complete <- merged_df[complete.cases(merged_df[, c("probability", "pI", "Mass_kDa" , "Length_AA")]), ]

# Regression: testscores ~ pI + Mass_kDa
model <- lm(probability ~ pI + Mass_kDa + Length_AA, data = df_complete)
...
```{r}
# Spearman correlation with length
cor(df_complete$predicted_prob, df_complete$pI, method = "spearman")
cor(df_complete$predicted_prob, df_complete$Length_AA, method = "spearman")
cor(df_complete$Length_AA, df_complete$pI, method = "spearman")
...
```{r}
# pearson with mass
cor(df_complete$predicted_prob, df_complete$pI, method = "pearson")
cor(df_complete$predicted_prob, df_complete$Mass_kDa, method = "pearson")
...
cor(df_complete$predicted_prob, df_complete$pI, method = "pearson")
cor(df_complete$predicted_prob, df_complete$Length_AA, method = "pearson")
cor(df_complete$Length_AA, df_complete$pI, method = "pearson")
```
