...
# smooth over the lines with spline() and
...
```{r}
first_two_components <- Ctrl_data$x[,1:2]
...
```{r}
first_two_components <- rnase_data$x[,1:2]
plot(first_two_components)
```
...
```{r}
#Dataframes speichern für lineare regression
dir.create("data", showWarnings = FALSE)
saveRDS(test_scores_combined_df, file = "data/test_scores_combined_df.rds")
saveRDS(Table_HS_RBP, file =  "data/Table_HS_RBP.rds")
saveRDS(Table_HS_Non_RBP, file =  "data/Table_HS_Non_RBP.rds")
```
...
```
```{r}
# Searman correlation with length
cor(df_complete$predicted_prob, df_complete$pI, method = "spearman")
...
# depict mean in plot
points(c(1, 2), c(mean_rbp, mean_nonrbp), pch = 19, col = "coral")
