...
```{r}
rsp_sva_df <- right_shift_proteins_df[,2:ncol(right_shift_proteins_df)]
str(rsp_sva_df)

apply(rsp_sva_df, 2, mean)

rsp_pca <- prcomp(rsp_sva_df, center = TRUE, scale = TRUE)
...
rsp_pca_var <- rsp_pca$sdev^2 # eigenvalues 
rsp_pca_var

rsp_pca_ve <- round(rsp_pca_var / sum(rsp_pca_var)*100,2) # wert in prozent auf zwei dezimal 
rsp_pca_ve

sort(rsp_pca_ve,decreasing=TRUE)
```
...
###weirder plot 
```{r}
biplot(rsp_pca, scale = 0, cex = 0.4)
```
...
#elbow 
wss <- sapply(1:25, function(k){
  kmeans(rsp_pca$x, centers = k, nstart = 25)$tot.withinss
})
...
# Nur die numerische Matrix aus dem PCA-Objekt extrahieren
pca_data <- rsp_pca$x  # oder ggf. pca_data <- na.omit(Ctrl_pca$x)
...
###kmeans 
```{r}
kmeans_result <- kmeans(rsp_pca$x, centers = 2, nstart = 25)
```
...
```{r}
rsp_data <- as.data.frame(rsp_pca$x)
rsp_data$cluster <- as.factor(kmeans_result$cluster)

library(ggplot2)

ggplot(rsp_data, aes(x = PC1, y = PC2, color = cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(title = "K-Means Cluster im PCA-Raum",
       x = "Hauptkomponente 1 (PC1)",
       y = "Hauptkomponente 2 (PC2)") +
...
```{r}
first_two_components <- rsp_data$x[,1:2]
plot(first_two_components)
```
