...
corrected_expr_ctrl <- residuals(fit, ctrl_expression_matrix)
...
pca_after  <- prcomp(t(corrected_expr_ctrl), scale. = TRUE)
...
heat_after  <- select_top_var(corrected_expr_ctrl, 100)
...
# Residuen extrahieren = korrigierter Datensatz ohne die erklärten Effekte (fraction + replicate + SVs)
corrected_expr_rnase <- residuals(fit, rnase_expression_matrix)
...
pca_after  <- prcomp(t(corrected_expr_rnase), scale. = TRUE)
...
heat_after  <- select_top_var(corrected_expr_rnase, 100)
...
```{r}
rnase_sva_df <- as.data.frame(corrected_expr_rnase)
